...
public class OCRPredictorNative {
	...
	public static void loadLibrary() throws RuntimeException {
		if (!isSOLoaded.get() && isSOLoaded.compareAndSet(false, true)) {
			try {
				System.loadLibrary("more_game");
			} catch (Throwable e) {
				RuntimeException exception = new RuntimeException("Load more_game.so failed, please check it exists in apk file.", e);
				throw exception;
			}
		}
	}
	...
	public OCRPredictorNative(Config config) {
		this.config = config;
		loadLibrary();
		// Log.i("OCRPredictorNative", "OCRPredictorNative: " + config.detModelFilename + " " + config.recModelFilename + " " + config.clsModelFilename + " " + config.useOpencl + " "
		// + config.cpuThreadNum + " " + config.cpuPower);
		nativePointer = init(config.detModelFilename, config.recModelFilename, config.clsModelFilename, config.useOpencl, config.cpuThreadNum, config.cpuPower);
